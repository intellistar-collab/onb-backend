...
export class SubscriptionController {
  ...
  async subscribe(@Body(ValidationPipe) body: CreateSubscriptionDto) {
    try {
  ...
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async getAllSubscriptions(@Query('page') page = 1, @Query('limit') limit = 10) {
    return this.subscriptionService.findAll(Number(page), Number(limit));
  }

  @Get('all')
  @ApiOperation({ summary: 'Get all subscribed emails (no pagination)' })
  @ApiResponse({ status: 200, description: 'Complete list of subscribed emails' })
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async getAllSubscriptionsUnpaginated() {
    return this.subscriptionService.findAllUnpaginated();
  }
  ...
  async sendEmailToSubscribers(@Body(new ValidationPipe()) body: SendBulkEmailDto) {
  ...
  @ApiResponse({ status: 200, description: 'Emails sent successfully to selected recipients' })
  @ApiResponse({ status: 400, description: 'Invalid email list or missing fields' })
  @ApiResponse({ status: 500, description: 'Failed to send emails' })
  @ApiBody({
  ...
  async sendEmailToSpecific(@Body(new ValidationPipe()) body: SendToSpecificDto) {
    return this.emailService.sendEmailsToSelected(body.emails, body.subject, body.message);
  }
  ...
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async verify(@Body(new ValidationPipe()) body: VerifySubscriptionDto) {
    const subscription = await this.subscriptionService.verifySubscription(body.token);
    console.log('subscription', subscription);

    await this.emailService.sendWelcomeEmail(subscription.data);
    if (subscription.data?.referrerEmail && !subscription.data?.followUpEmailSent) {
      ...
      await this.emailService.sendEmailSuccessfulReferral(subscriptionData?.existing);
    }
