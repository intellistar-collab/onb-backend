...
export class AuthService {
  ...
  async validateUser(email: string, password: string) {
    ...
    if (user.requiresOTP === true) {
      return { id: user.id, email: user.email, role: user.role, requiresOTP: true };
    }

    return { id: user.id, email: user.email, role: user.role, requiresOTP: false };
  }
  ...
  async verifyOTP(email: string, otp: string) {
    ...
    await this.usersService.updateUser(user.id, { otp: null, otpExpiry: null, requiresOTP: false });
    const refreshToken = this.generateRefreshToken();
    const refreshTokenExpiry = new Date();
    refreshTokenExpiry.setDate(refreshTokenExpiry.getDate() + 7);

    await this.usersService.updateUser(user.id, { refreshToken, refreshTokenExpiry });
